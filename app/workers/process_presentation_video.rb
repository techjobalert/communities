class ProcessPresentationVideo
  ...
  def self.perform(present_attachment_id, recorded_attachment_id, params)
    ...
    if present_attachment.attachment_type == "presentation_video" and params["playback_points"]
      ...
      p "ffmpeg -shortest -ar 44100 -acodec pcm_s16le -f s16le -ac 2 -i /dev/zero -i #{file_no_sound} -r #{frame_rate} -qmax 51 -qmin 11 -vcodec libvpx -acodec libvorbis #{final} -map 1:0 -map 0:0"
      %x[ffmpeg -shortest -ar 44100 -acodec pcm_s16le -f s16le -ac 2 -i /dev/zero -i #{file_no_sound} -r #{frame_rate} -qmax 51 -qmin 11 -vcodec libvpx -acodec libvorbis #{final} -map 1:0 -map 0:0]
